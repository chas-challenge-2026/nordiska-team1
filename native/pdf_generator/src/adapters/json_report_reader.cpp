...
Report read_report_json(const std::filesystem::path& input_path) {
    ...
    for (std::sregex_iterator it(json.begin(), json.end(), object_pattern), end; it != end; ++it) {
        ...
        report.transactions.push_back(Transaction{
            required_string(object, "date"), required_string(object, "type"),
            required_string(object, "currency"), required_integer(object, "amount_minor")});
    }
    return report;
}
} // namespace nordiska
