...
std::string format_minor_units(std::int64_t amount_minor,
                               const std::string& currency) {
    const bool negative = amount_minor < 0;
    const std::uint64_t absolute = negative
        ? static_cast<std::uint64_t>(-(amount_minor + 1)) + 1
    ...
    formatted << absolute / 100 << '.' << std::setw(2) << std::setfill('0')
              << absolute % 100 << ' ' << currency;
    ...
    for (const Transaction& transaction : report.transactions) {
        content << "0 -18 Td\n("
                << pdf_escape(transaction.date + " | " + transaction.type +
                              " | " + format_minor_units(transaction.amount_minor,
...
void MinimalPdfRenderer::render(const Report& report,
