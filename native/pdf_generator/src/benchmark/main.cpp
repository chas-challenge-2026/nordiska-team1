...
std::unique_ptr<nordiska::IPdfRenderer> make_renderer(std::string_view name) {
    if (name == "haru") return std::make_unique<nordiska::LibHaruPdfRenderer>();
...
Options parse_options(int argc, char* argv[]) {
    ...
    for (int index = 2; index < argc; ++index) {
        ...
        if (argument == "--renderer") options.renderer = value("--renderer");
        else if (argument == "--iterations") options.iterations = std::stoull(value("--iterations"));
        else if (argument == "--warmups") options.warmups = std::stoull(value("--warmups"));
        else if (argument == "--limit") options.limit = std::stoull(value("--limit"));
        else if (argument == "--sample-count") options.sample_count = std::stoull(value("--sample-count"));
        else if (argument == "--output-dir") options.output_directory = value("--output-dir");
        else if (argument == "--delete-output") options.delete_output = true;
...
std::vector<std::filesystem::path> discover(const Options& options) {
    ...
    if (paths.empty()) throw std::runtime_error("input directory contains no JSON reports");
    return paths;
}
...
PhaseMetrics timed(const LoadedCorpus& corpus, std::optional<std::size_t> output_bytes, Function&& function) {
    ...
    return {.seconds = seconds, .reports = corpus.reports.size(), .transactions = corpus.transactions,
...
std::vector<NamedMetrics> measure(const std::string& renderer_name,
    ...
    for (const auto& report : corpus.reports) rendered.push_back(render_to_memory(report, *renderer));
    std::size_t bytes = 0;
    for (const auto& value : rendered) bytes += value.size();
    ...
    results.push_back({"persistence", timed(corpus, bytes, [&] {
                           for (std::size_t index = 0; index < rendered.size(); ++index) {
                               nordiska::FileByteSink sink(persistence_dir / ("report-" + std::to_string(index) + ".pdf"));
    ...
    if (options.delete_output) std::filesystem::remove_all(persistence_dir);
    return results;
}
...
void write_csv_row(std::ostream& output, const std::string& renderer, std::size_t iteration,
    ...
    if (metrics.output_bytes) output << *metrics.output_bytes;
    else output << "NA";
    output << '\n';
}
...
void write_human_report(const std::filesystem::path& path, const Options& options,
    ...
    if (!output) throw std::runtime_error("could not create benchmark report: " + path.string());
    output << "# Nordiska native PDF benchmark\n\n"
              "- Input corpus: `" << options.input_directory.string() << "`\n"
              "- Reports: " << report_count << "\n"
              "- Transactions: " << transaction_count << "\n"
              "- Measured iterations: " << options.iterations << "\n"
              "- Warmup iterations: " << options.warmups << "\n"
              "- Output directory: `" << run_directory.string() << "`\n\n"
              "The timings are wall-clock durations. Throughput is calculated from the full corpus count."
              " Input loading includes file reading and JSON parsing. Persistence writes already-rendered"
              " bytes through `FileByteSink`; end-to-end includes loading, rendering, and persistence.\n\n"
              "## Summary\n\n";
...
int main(int argc, char* argv[]) {
    try {
        ...
        options.output_directory /= "run-" + std::to_string(
        ...
        if (!csv) throw std::runtime_error("could not create benchmark CSV: " + csv_path.string());
        ...
        csv.close();
        if (records.empty()) throw std::runtime_error("benchmark produced no measured results");
        ...
        write_human_report(report_path, options, options.output_directory, records,
                           report_count, transaction_count);
        ...
        if (options.delete_output) std::filesystem::remove_all(options.output_directory);
        return 0;
