...
int main() {
    ...
    const int status = nordiska_document_generate_json(reinterpret_cast<const uint8_t*>(json.data()), json.size(),
    ...
    require(std::memcmp(pdf.data(), "%PDF-1.3", 8) == 0, "C API did not use the Haru production composition");

    const std::string invalid_json = "{}";
    const int invalid_status =
        nordiska_document_generate_json(reinterpret_cast<const uint8_t*>(invalid_json.data()), invalid_json.size(),
                                        save_pdf, &pdf, error, sizeof(error));
    require(invalid_status == NORDISKA_DOCUMENT_INVALID_INPUT, "invalid report did not return invalid-input status");
    ...
    const int invalid_report_status =
        nordiska_document_generate_json(reinterpret_cast<const uint8_t*>(invalid_report.data()), invalid_report.size(),
                                        save_pdf, &pdf, error, sizeof(error));
    require(invalid_report_status == NORDISKA_DOCUMENT_INVALID_INPUT,
            "invalid domain report did not return invalid-input status");

    const int callback_status = nordiska_document_generate_json(reinterpret_cast<const uint8_t*>(json.data()),
                                                                json.size(), reject_pdf, nullptr, error, sizeof(error));
